...
import { ISelectable, IMovable } from "../tools/Selector";
...
export interface ControlPointHost {
...
export class ControlPoint extends Element implements IMovable {
    ...
    public constructor(host: ControlPointHost & Element, x: number, y: number, options?: {radius?: number, cursor?: ControlPointCursor}) {
        super(host.parent);
        this.host = host;
    ...
    public get cursor(): Cursor {
        switch(this._cursor) {
            case ControlPointCursor.TOP_LEFT: return Cursor.NW_RESIZE;
            case ControlPointCursor.TOP: return Cursor.N_RESIZE;
            case ControlPointCursor.TOP_RIGHT: return Cursor.NE_RESIZE;
            case ControlPointCursor.LEFT: return Cursor.W_RESIZE;
            case ControlPointCursor.MOVE: return Cursor.MOVE;
            case ControlPointCursor.RIGHT: return Cursor.E_RESIZE;
            case ControlPointCursor.BOTTOM_LEFT: return Cursor.SW_RESIZE;
            case ControlPointCursor.BOTTOM: return Cursor.S_RESIZE;
            case ControlPointCursor.BOTTOM_RIGHT: return Cursor.SE_RESIZE;
        }
    }
    ...
    public finishMove(x: number, y: number) {
        this.isMoving = false;
        this._x = x;
        this._y = y;
